...
if torch.cuda.is_available():
    ...
    HIGH_END_GPU = (
        gpu_mem_gb >= 16.0
        or props.major >= 8
        or (gpu_mem_gb >= 12.0 and props.major >= 7)
    )
...
API_URL = os.environ.get("ORPHEUS_API_URL", "http://172.27.176.1:1234/v1/completions")
if not API_URL:
...
class PerformanceMonitor:
    ...
    def report(self) -> None:
        ...
        _chunks_per_sec = self.audio_chunks / elapsed
...
async def generate_tokens_from_api_async(
    ...
    """
    Asynchronously generates tokens from text using an OpenAI-compatible API with
    optimized streaming and retry logic.  Uses aiohttp for non-blocking requests.
    """
    ...
    # Create the request payload (model field may not be required by some endpoints)
    payload = {
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        text = await response.text()  # Get the error text
                        ...
                        if response.status >= 500:
                            ...
                            continue
                        return  # Exit the function on non-retriable errors

                    # Process the streamed response
                    _buffer = ""
                    token_counter = 0

                    # Use response.content to get an async content reader.
                ...
                if audio_chunk:
                    ...
                    if current_time - last_log_time >= 3.0:  # Every 3 seconds
                        ...
                        if elapsed > 0:
                            ...
                            print(
                                f"Audio generation rate: {chunks_per_sec:.2f} chunks/second"
                            )
                        last_log_time = current_time
...
def stitch_wav_files(input_files, output_file, crossfade_ms=50):
    ...
    print(
        f"Stitching {len(input_files)} WAV files together with {crossfade_ms}ms crossfade"
    )
            ...
            with wave.open(input_file, "rb") as wav:
                ...
                if i == 0:
                    ...
                    if (
                        ...
                        and len(audio) >= crossfade_samples
                    ):
                        ...
                        print(
                            f"Segment {i} too short for crossfade, concatenating directly"
                        )
...
def main():
    ...
    # Generate speech
    start_time = time.time()
    _audio_segments = generate_speech_from_api(
        prompt=prompt,
        voice=args.voice,
